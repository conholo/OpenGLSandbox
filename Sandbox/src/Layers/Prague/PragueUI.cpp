...
void PragueUI::DrawInputUI(PragueInput* input, const PragueSkyModel::AvailableData& data, bool* valueChanged)
{
	...
	*valueChanged = false;
	PragueSkyModel::AvailableData available = data;

	*valueChanged |= ImGui::Checkbox("ACES Tone Map", &input->ApplyToneMap);
	...
	*valueChanged |= ImGui::SliderFloat("exposure", &input->Exposure, -25.0f, 25.0f, "%.1f");
}
