...
{
	AnimationParam(const std::string& name, float* ptr, float min, float max, float speed)
		:Name(name), ParamPtr(ptr), Min(min), Max(max), Speed(speed)
	{
	}
...
struct PragueInput
{
	glm::vec4 OriginData = glm::vec4(0.0f);
    float Albedo = 0.5;
...
public:
    ...
    const size_t GetTransVSize() const { return sizeof(float) * dataTransV.size(); }
    const std::vector<float>& GetTransV() const { return dataTransV; }

    const size_t GetTransUSize() const { return sizeof(float) * dataTransU.size(); }
    const std::vector<float>& GetTransU() const { return dataTransU; }

    const size_t GetTransAltitudeSize() const { return sizeof(double) * altitudesTrans.size(); }
	const std::vector<double>& GetTransAltitudes() const { return altitudesTrans; }

    const size_t GetTransVisibilitiesSize() const { return sizeof(double) * visibilitiesTrans.size(); }
	const std::vector<double>& GetTransVisibilities() const { return visibilitiesTrans; }

	const std::vector<float>& GetPolarizationData() const { return dataPol; }
	const size_t GetPolarizationDataSize() const { return sizeof(float) * dataPol.size(); }
	const Metadata& GetPolarizationMetaData() const { return metadataPol; }

	const size_t GetPolSunMetaDataBreaksSize() const { return metadataPol.sunBreaks.size() * sizeof(double); }
	const std::vector<double>& GetPolSunMetaDataBreaks() const { return metadataPol.sunBreaks; }

	const size_t GetPolZenithMetaDataBreaksSize() const { return metadataPol.zenithBreaks.size() * sizeof(double); }
	const std::vector<double>& GetPolZenithMetaDataBreaks() const { return metadataPol.zenithBreaks; }

	const size_t GetPolEmphMetaDataBreaksSize() const { return metadataPol.emphBreaks.size() * sizeof(double); }
	const std::vector<double>& GetPolEmphMetaDataBreaks() const { return metadataPol.emphBreaks; }

	const size_t GetVisibilitiesPolarizationSize() const { return sizeof(double) * visibilitiesRad.size(); }
	const std::vector<double>& GetVisibilitiesPolarization() const { return visibilitiesRad; }

	const size_t GetAlbedosPolarizationSize() const { return sizeof(double) * albedosRad.size(); }
	const std::vector<double>& GetAlbedosPolarization() const { return albedosRad; }

	const size_t GetAltitudesPolarizationSize() const { return sizeof(double) * altitudesRad.size(); }
	const std::vector<double>& GetAltitudesPolarization() const { return altitudesRad; }

	const size_t GetElevationsPolarizationSize() const { return sizeof(double) * elevationsRad.size(); }
	const std::vector<double>& GetElevationsPolarization() const { return elevationsRad; }


    const int GetADim() const { return aDim; }
    const int GetDDim() const { return dDim; }
    const int GetTransRank() const { return rankTrans; }
