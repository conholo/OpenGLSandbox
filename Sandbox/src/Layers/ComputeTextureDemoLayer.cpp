#include "Layers/ComputeTextureDemoLayer.h"
#include <glad/glad.h>

ComputeTextureDemoLayer::ComputeTextureDemoLayer()
...
void ComputeTextureDemoLayer::OnAttach()
{
...
void ComputeTextureDemoLayer::OnDetach()
{

}

void ComputeTextureDemoLayer::OnUpdate(float deltaTime)
{
	...
	Engine::ShaderLibrary::Get("FBMCompute")->Bind();
	Engine::ShaderLibrary::Get("FBMCompute")->UploadUniformFloat4("u_Params", glm::vec4(m_CurrentPersistence, m_CurrentLacunarity, 8, m_CurrentScale));
	Engine::ShaderLibrary::Get("FBMCompute")->UploadUniformFloat2("u_Offset", m_Offset);
	Engine::ShaderLibrary::Get("FBMCompute")->UploadUniformFloat4("u_NoiseColor", m_NoiseColor);
	Engine::ShaderLibrary::Get("FBMCompute")->UploadUniformFloat("u_Time", elapsed);
	Engine::ShaderLibrary::Get("FBMCompute")->DispatchCompute(m_TestComputeTexture->GetWidth(), m_TestComputeTexture->GetHeight(), 1);
...
void ComputeTextureDemoLayer::OnEvent(Engine::Event& event)
{
	Engine::EventDispatcher dispatcher(event);

	dispatcher.Dispatch<Engine::KeyPressedEvent>(BIND_FN(ComputeTextureDemoLayer::OnKeyPressed));

	m_Camera.OnEvent(event);
}

bool ComputeTextureDemoLayer::OnKeyPressed(Engine::KeyPressedEvent& event)
{
