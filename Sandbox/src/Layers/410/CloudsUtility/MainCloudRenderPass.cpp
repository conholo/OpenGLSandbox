...
void MainCloudRenderPass::ExecutePass(const Engine::Camera& camera, const Engine::Ref<MainCloudPassData>& passData)
{
	...
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformFloat("u_CloudScrollOffsetSpeed", passData->AnimationSettings->CloudScrollOffsetSpeed);
	...
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformFloat4("u_ShapeNoiseWeights", passData->BaseShapeSettings->ShapeNoiseWeights);
	...
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformInt("u_DisplayIndex", passData->UI->GetActiveUIType() == CloudUIType::BaseShape ? 1 : 0);
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformInt("u_DisplayTexture3D", 2);
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformInt("u_DisplayTexture2D", 3);
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformBool("u_ShowAlpha", passData->UI->GetTextureDisplaySettings()->ShowAlpha);
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformBool("u_ShowAllChannels", passData->UI->GetTextureDisplaySettings()->DrawAllChannels);
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformBool("u_GreyScale", passData->UI->GetTextureDisplaySettings()->EnableGreyScale);
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformInt("u_Texture", 0);
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformFloat("u_DepthSlice", passData->UI->GetTextureDisplaySettings()->DepthSlice);
	m_CloudQuad->GetEntityRenderer()->GetShader()->UploadUniformFloat4("u_ChannelWeights", passData->UI->GetTextureDisplaySettings()->ChannelWeights);
